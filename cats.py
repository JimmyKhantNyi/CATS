...
def time_per_word(words, timestamps_per_player):
    ...
    "*** YOUR CODE HERE ***"
    times = []
    for time_player in timestamps_per_player:
        new_list = []
        for i in range(0, len(time_player) - 1):
            new_list += [time_player[i+1] - time_player[i]]
            i += 1
        times += [new_list]
...
def fastest_words(match):
    ...
    for i in word_indices:
        word = [current_word[i] for current_word in times]
        player_number = min(range(len(word)), key = lambda x: word[x])
        new_list[player_number].append(words[i])
    return new_list
